...
export class NavBarAnimations {
    ...
    initNavLinks = () => {
        $(() => {
            $(this._links).each((_, e) => {
                ...
                $(e).on('click', (e) => { if ($(this).hasClass(this._disabledClass)) e.preventDefault(); });
                $(e).on('mouseenter', () => gsap.fromTo(image, { opacity: 0 }, { display: 'block', opacity: 1, duration: 0.5 }));
    ...
    disableNavLinks = () => {
        $(this._links).each((_, e) => { $(e).addClass(this._disabledClass); })
    }

    enableNavLinks = () => {
        $(this._links).each((_, e) => { $(e).removeClass(this._disabledClass); })
    }
    ...
    scrollButtonInit = () => {
        ...
        ScrollTrigger.create({
            trigger: openingHero,
            start: 'bottom 50%',
            onEnter() { gsap.to(scrollButton, { opacity: 1, duration: 1 }) },
            onLeaveBack() { gsap.to(scrollButton, { opacity: 0, duration: 1 }) }
        })
    }
}
